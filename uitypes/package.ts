...
export namespace UIPackage {
  ...
  export function load(packagename: string, basePath: string): UIPackage | undefined {
    ...
    resourcesNode.elements.forEach(({ name, attributes }) => {
      // 解析非自定义组件, 直接使用内置扩展类型
      if (name !== "component") {
        const type = fairygui.toFairyguiType(name);
      ...
      // 解析自定义组件引用类型
      const pathRef = fairygui.toRef(attributes.path);
      const ref =
        pathRef.length > 0
          ? `${packagename}.${pathRef}.${component.publishName}`
          : `${packagename}.${component.publishName}`;
  ...
  export function compile(
    pkg: UIPackage,
    getPackage: (packageID: string) => UIPackage | undefined,
    format?: boolean
  ): string {
    // 获取组件引用路径
